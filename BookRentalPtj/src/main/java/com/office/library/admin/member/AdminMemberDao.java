...
public class AdminMemberDao {
	...
	public boolean isAdminMember(String a_m_id) {
	 ...
	 public int insertAdminAccount(AdminMemberVo adminMemberVo) {
		 ...
		 * AdminMemberService에서 보낸 매개변수를 받는다
		 ...
		 * 
		 * 
		 * List<AdminMemberVo> adminMemberVos = new ArrayList<AdminMemberVo>(); // 리스트
		 * 타입의 객체 생성
		 * 
		 * try { // sql문, sql 결과값의 타입, ?에 들어갈 변수 adminMemberVos =
		 * jdbcTemplate.query(sql, new RowMapper<AdminMemberVo>() { //adminMemberVos에 저
		 * 쿼리의 결과를 담는다!
		 * 
		 * @Override public AdminMemberVo mapRow(ResultSet rs, int rowNum) throws
		 * SQLException { //mapRow()는 ResultSet과 행의 개수를 파라미터로 받는다
		 * 
		 * // rs에 있는 데이터를 adminMemberVo객체의 setter를 이용해서 저장 AdminMemberVo adminMemberVo =
		 * new AdminMemberVo();
		 * 
		 ...
		 * 
		 * return adminMemberVo; // 저장이 끝나면 AdminMemberVo의 객체를 반환! >> 이 반환된 객체는 List
		 * 타입으로 저장된 후 adminMemberVos에 할당된다.
		 * 
		 * }
		 * 
		 * }, adminMemberVo.getA_m_id()); // 여기 getA_m_id()가 위에 sql문 ?에 들어감 //암호화된 문자열을
		 * 비교하는 메서드 :passwordEncoder의 matches() if
		 * (!passwordEncoder.matches(adminMemberVo.getA_m_pw(), // matches(a,b) 결과가
		 * false라면 비밀번호가 안 맞는걸로 판단하여 if문 true타고 adminMemberVos.get(0).getA_m_pw()))
		 * adminMemberVos.clear(); //조회된 데이터는 삭제
		 * 
		 * } catch (Exception e) { //시스템적인 에러 e.printStackTrace(); }
		 * 
		 * return adminMemberVos.size() > 0 ? adminMemberVos.get(0) : null; // 0보다 크면
		 * 로그인 인증에 성공 및 조회된 관리자 정보를 서비스에 반환 } // 길이가 0 이하라면 로그인 인증 실패 및 서비스에 null 반환
		 */
	 ...
	 public AdminMemberVo selectAdmin(AdminMemberVo adminMemberVo) {
	 	...
	 	public List<AdminMemberVo> selectAdmins() {
			...
			return adminMemberVos;
	 		...
	 		public int updateAdminAccount(int a_m_no) { // 관리자 승인
	 		...
	 		public int updateAdminAccount(AdminMemberVo adminMemberVo) { //회원 정보 업데이트 , 같은 클래스내에서 동일한 메서드를 재선언 >> 오버로딩
	 			System.out.println("[AdminMemberDao] updateAdminAccount()");
